...
func (t Transition) Enter(exec statescript.Executor, req *client.ApiRequest, serverURL string) error {
	if t == ToNone {
		return nil
	}
	...
	if err := exec.ExecuteAll(name, "Enter", ignoreErrors(t, "Enter"), req, serverURL); err != nil {
		return errors.Wrapf(err, "error running enter state script(s) for %v state", t)
	}
	return nil
}

func (t Transition) Leave(exec statescript.Executor, req *client.ApiRequest, serverURL string) error {
	if t == ToNone {
		return nil
	}
	...
	if err := exec.ExecuteAll(name, "Leave", ignoreErrors(t, "Leave"), req, serverURL); err != nil {
		return errors.Wrapf(err, "error running leave state script(s) for %v state", t)
	}
	return nil
}

func (t Transition) Error(exec statescript.Executor, req *client.ApiRequest, serverURL string) error {
	if t == ToNone {
		return nil
	}
	...
	if err := exec.ExecuteAll(name, "Error", true, req, serverURL); err != nil {
